...
describe("VolunteerHub Router", () => {
  ...
  describe("health", () => {
    it("should return healthy status", async () => {
      const result = { status: "healthy", module: "volunteerHub", version: "3.0.0" };
      expect(result.status).toBe("healthy");
      expect(result.module).toBe("volunteerHub");
    });
  });
